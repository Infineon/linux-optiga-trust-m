...
echo "provisioning for AES key Protected Update"
...
echo "Step3: Provisioning metadata for 0x$TARGET_AES_OID"
echo "Set AES protected update for 0x$TARGET_OID (Provision for Protected Update)"
echo $TARGET_AES_OID_META | xxd -r -p > targetOID_metadata.bin
echo "Printout targetOID_metadata.bin"
xxd targetOID_metadata.bin
echo "Write targetOID_metadata.bin as metadata of 0x$TARGET_AES_OID"
$EXEPATH/trustm_metadata -w 0x$TARGET_AES_OID -F targetOID_metadata.bin
echo "Read out metadata of 0x$TARGET_AES_OID"
