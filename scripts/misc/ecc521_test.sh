...
$EXEPATH/trustm_ecc_sign -X -k 0x$KEY_OID -o testsignature_521.bin -i mydata.txt -H sha512

echo "------> verify Trust M signature with openssl"
openssl dgst -verify test_pub_$KEY_OID.pem -keyform pem -sha512 -signature testsignature_521.bin mydata.txt

echo "------> verify with Trust M"
$EXEPATH/trustm_ecc_verify -X -i mydata.txt -s testsignature_521.bin -p test_pub_$KEY_OID.pem -H sha512
