...
openssl ecparam --provider trustm_provider --provider default -name prime256v1:0xe0f3 -genkey -noout
echo "Generate the customized key file"
openssl pkey -provider trustm_provider -provider default -propquery provider=trustm -in 0xe0f3:^ -out key.pem
...
openssl ec -in key.pem -pubout -conv_form uncompressed -out e0f3_pub.pem
...
openssl pkeyutl -verify -pubin -inkey e0f3_pub.pem -rawin -in testdata.txt -sigfile testdata.sig
