...
int iot_tls_connect(Network *pNetwork, TLSConnectParams params) {
	...
	ENGINE	*e;
	const char 	*engine_name = "trustm_engine";
	EVP_PKEY	*pkey;
	UI_METHOD	*ui_method;
	char  *owner_auth=NULL;
	...
	if (!e){
		DEBUG("Load Trust M  Engine Fail!!!");
	} else {
		DEBUG("Engine ID : %s",ENGINE_get_id(e));

		if (!ENGINE_init(e)){
			DEBUG("Cannot Init Trust M Engine!!");
		}

		if (!ENGINE_set_default(e, ENGINE_METHOD_ALL)){
			DEBUG("Cannot use Trust M Engine!!");
		}

		//DEBUG("Load Trust M Device Certificate");
		...
		//DEBUG("Load Trust M Generated Key");
